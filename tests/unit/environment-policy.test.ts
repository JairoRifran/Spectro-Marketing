...
describe("automation environment guard",()=>{
  ...
  it("uses the mock provider when Vercel stores AI_PROVIDER as an empty value",()=>{process.env.AI_PROVIDER="";expect(configuredAgentProviderName()).toBe("mock");expect(getAgentProvider().name).toBe("mock")});
