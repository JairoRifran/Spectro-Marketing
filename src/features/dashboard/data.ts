...
export async function getHqData(): Promise<HqData> {
  ...
  const empty: HqData = { ...demoData, mode:"live", organizationName:"Sin organización", needsOnboarding:true, objective:null, agents:[], counts:{completed:0,running:0,queued:0,approval:0,failed:0}, approval:null, activity:[], health:{healthy:false,lastDispatch:null} };
  const context = await getOrganizationContext();
  if (!context) return empty;
  const { db, orgId } = context;
  const [objectiveResult,agentsResult,tasksResult,approvalResult,activityResult,healthResult,pendingApprovalResult] = await Promise.all([
  ...
  return {mode:"live",organizationName:context.orgName,needsOnboarding:false,objective,agents,counts:{completed:tasks.filter(t=>t.status==="completed").length,running:tasks.filter(t=>t.status==="running").length,queued:tasks.filter(t=>t.status==="queued").length,approval:pendingApprovalResult.count??0,failed:tasks.filter(t=>t.status==="failed").length},approval:approvalResult.data?{id:approvalResult.data.id,title:approvalTask?.title??"Solicitud de aprobación",reason:approvalResult.data.reason,risk:approvalResult.data.risk_level,agentName:proposer?.name??"Agente",agentRole:proposer?.role??"Equipo"}:null,activity:(activityResult.data??[]).map(item=>({id:item.id,summary:item.summary,action:item.action,createdAt:item.created_at})),health:{healthy,lastDispatch}};
}
