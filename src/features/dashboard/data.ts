import { PENDING_APPROVAL_STATUS } from "@/features/approvals/count";
...
export type HqData = {
  mode:"demo"|"live"; organizationName:string; needsOnboarding:boolean; objective:null|{title:string;metric:string;baseline:number|null;target:number;market:string|null;priority:string;deadline:string|null};
  agents:HqAgent[]; counts:{completed:number;running:number;queued:number;approval:number;failed:number};
  campaigns:{active:number;developing:number;waitingApproval:number};
...
const demoData: HqData = {
  mode:"demo", organizationName:"Northstar Urban", needsOnboarding:false, objective:{title:"Aumentar registros calificados un 30%",metric:"Registros calificados",baseline:1240,target:1612,market:"Uruguay",priority:"high",deadline:"2026-10-12"},
  agents:[
  ...
  ], counts:{completed:8,running:2,queued:3,approval:1,failed:0},campaigns:{active:0,developing:0,waitingApproval:1},
...
export async function getHqData(): Promise<HqData> {
  ...
  const empty: HqData = { ...demoData, mode:"live", organizationName:"Sin organización", needsOnboarding:true, objective:null, agents:[], counts:{completed:0,running:0,queued:0,approval:0,failed:0},campaigns:{active:0,developing:0,waitingApproval:0}, approval:null, activity:[], health:{healthy:false,lastDispatch:null} };
  const context = await getOrganizationContext();
  if (!context) return empty;
  const { db, orgId } = context;
  const [objectiveResult,agentsResult,tasksResult,approvalResult,activityResult,healthResult,pendingApprovalResult,campaignResult] = await Promise.all([
    ...
    db.from("approvals").select("id,reason,risk_level,tasks(title,assigned_agent_id)").eq("organization_id",orgId).eq("status",PENDING_APPROVAL_STATUS).order("created_at").limit(1).maybeSingle(),
    ...
    db.from("approvals").select("id",{count:"exact",head:true}).eq("organization_id",orgId).eq("status",PENDING_APPROVAL_STATUS),
    db.from("campaigns").select("status").eq("organization_id",orgId).limit(200),
  ]);
  ...
  const campaigns=campaignResult.data??[];return {mode:"live",organizationName:context.orgName,needsOnboarding:false,objective,agents,counts:{completed:tasks.filter(t=>t.status==="completed").length,running:tasks.filter(t=>t.status==="running").length,queued:tasks.filter(t=>t.status==="queued").length,approval:pendingApprovalResult.count??0,failed:tasks.filter(t=>t.status==="failed").length},campaigns:{active:campaigns.filter(item=>item.status==="active").length,developing:campaigns.filter(item=>["draft","researching","strategy"].includes(item.status)).length,waitingApproval:campaigns.filter(item=>item.status==="ready").length},approval:approvalResult.data?{id:approvalResult.data.id,title:approvalTask?.title??"Solicitud de aprobación",reason:approvalResult.data.reason,risk:approvalResult.data.risk_level,agentName:proposer?.name??"Agente",agentRole:proposer?.role??"Equipo"}:null,activity:(activityResult.data??[]).map(item=>({id:item.id,summary:item.summary,action:item.action,createdAt:item.created_at})),health:{healthy,lastDispatch}};
}
