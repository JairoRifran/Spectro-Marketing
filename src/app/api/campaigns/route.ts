...
export async function POST(request:Request){
  ...
  const{data:campaign,error}=await db.from("campaigns").insert({organization_id:context.orgId,objective_id:objective.id,name,slug,status:"draft",business_goal:objective.title,campaign_type:"integrated",target_audience:parsed.data.specificAudience||null,start_date:parsed.data.startDate??null,end_date:parsed.data.endDate??null,budget:parsed.data.budget??null,constraints:parsed.data.constraints,priority:"high",created_by_type:"user",created_by_user_id:context.user.id}).select("id").single();
