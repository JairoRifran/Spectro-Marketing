...
export class MockProvider implements AgentProvider {
  ...
  async run(context: AgentContext): Promise<AgentResult> {
    if (context.task.type === "test.fail.retryable") {
      const failures = typeof context.task.input.failuresBeforeSuccess === "number" ? context.task.input.failuresBeforeSuccess : Number.POSITIVE_INFINITY;
      if (context.task.attempt_count <= failures) throw Object.assign(new Error("Deterministic retry test"), { retryable: true });
    }
