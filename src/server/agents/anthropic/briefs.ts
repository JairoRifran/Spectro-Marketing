...
export const BRIEFS: Record<string, Brief> = {
  ...
  "campaign.research": {
    ...
    // Measured, not chosen, twice over: this stage exceeded the deadline at high effort and
    // exceeded it again at medium, on a six-thousand token budget. Its schema is a dozen
    // parallel lists -- breadth, not depth, and the kind of work that grows with thinking time
    // without getting better. Reading a context and organising what is in it is not where a
    // reasoning budget earns anything. The draft, which is genuinely one hard judgement, still
    // runs high and finishes comfortably.
    effort: "low",
    system: role(
